...
- **建议悬浮窗** — 一个始终置顶的小窗口，浮在游戏画面上显示 AI
  建议的切牌，你就不用再切出游戏去看主窗口了。可以拖到任意位置、
  可以调整大小，关掉 Akagi 再打开，它还会回到原来的位置。
- **两种抓包模式**
...
Akagi 的建议平常显示在主窗口里，但你一开始打牌，主窗口就被游戏挡
住了。**建议悬浮窗**是一个小窗口，会浮在**游戏画面的上面**，列出 AI
建议的切牌，让你不用切出游戏也看得到。

到 **设置 → 建议悬浮窗 → 显示悬浮窗** 打开它，然后点 **保存**，它会
立刻出现。

- **移动** — 用鼠标直接把它拖到屏幕上任意位置。
- **调整大小** — 拖拽它的边缘。
- **关闭** — 点右上角的 **×**。想再打开，回设置里重新勾选即可。

Akagi 会记住你把它放在哪、调成多大，下次启动时直接还原。

有三个设置可以调整：

| 设置 | 作用 |
|---|---|
| **显示几个建议** | 要列出几个候选切牌（1～5 个）。 |
| **不透明度** | 悬浮窗有多透明，让你还能看见底下的牌桌。 |
| **保持在其他窗口之上** | 默认开启。如果你想把它放到第二个显示器，置顶反而碍事，就把这个关掉。 |

> **悬浮窗没有出现在游戏上面？**
> 你的游戏几乎可以肯定是开在“**独占全屏**”模式。在 Windows 上，独占
> 全屏会盖住所有置顶窗口——这是操作系统的规则，Akagi 没办法绕过。
> 把游戏改成**无边框**或**窗口**模式，悬浮窗就会出现了。
