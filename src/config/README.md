...
- `overlay` (`overlay.rs`) — the always-on-top suggestion window. `enabled` (default **true**) is its persisted lifecycle switch: `ipc::overlay::reconcile` opens or closes the window to match it at startup and on every `update_config` / `set_overlay_enabled`. The overlay's own close button closes only the current window and leaves `enabled` intact, so it returns on the next launch. A `config.toml` written before this section existed therefore gets the overlay switched on, which is the intent. `top_n` and `opacity` are bounds-checked by `clamped_top_n()` / `clamped_opacity()` rather than trusted, because `config.toml` is a text file a user can put `top_n = 0` into — which would render an overlay with no rows and no explanation.
