...
- **建議懸浮視窗** — 一個永遠置頂的小視窗，浮在遊戲畫面上顯示
  AI 建議的切牌，你就不用再切出遊戲去看主視窗了。可以拖到任何
  地方、可以調整大小，關掉 Akagi 再打開，它還會回到原本的位置。
- **兩種抓包模式**
...
Akagi 的建議平常顯示在主視窗裡，但你一開始打牌，主視窗就被遊戲蓋
住了。**建議懸浮視窗**是一個小視窗，會浮在**遊戲畫面的上面**，列出
AI 建議的切牌，讓你不用切出遊戲也看得到。

到 **設定 → 建議懸浮視窗 → 顯示懸浮視窗** 打開它，然後按 **儲存**，
它會立刻出現。

- **移動** — 用滑鼠直接把它拖到畫面上任何位置。
- **改變大小** — 拖曳它的邊緣。
- **關閉** — 點右上角的 **×**。想再打開，回設定裡重新勾選即可。

Akagi 會記住你把它放在哪、調成多大，下次開啟時直接還原。

有三個設定可以調整：

| 設定 | 作用 |
|---|---|
| **顯示幾個建議** | 要列出幾個候選切牌（1～5 個）。 |
| **不透明度** | 懸浮視窗有多透明，讓你還能看見底下的牌桌。 |
| **保持在其他視窗上方** | 預設開啟。如果你想把它放到第二個螢幕，置頂反而礙事，就把這個關掉。 |

> **懸浮視窗沒有出現在遊戲上面？**
> 你的遊戲幾乎可以確定是開在「**獨佔全螢幕**」模式。在 Windows 上，
> 獨佔全螢幕會蓋掉所有置頂視窗——這是作業系統的規則，Akagi 沒辦法
> 繞過。把遊戲改成**無邊框**或**視窗**模式，懸浮視窗就會出現了。
